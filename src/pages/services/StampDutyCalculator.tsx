...
function calculateSDLT_ENG_WALES_LIKE({
  ...
  // UI estimate based on common SDLT bands:
  // England & Northern Ireland (SDLT) - simplified for display.
  // This is not financial advice; connect API for authoritative calculation.
  const bands: Array<{
    from: number;
    to: number | null;
    rate: number;
    ...
    ? [
        { from: 0, to: 425000, rate: 0, label: "0% (first-time buyer threshold)" },
        { from: 425000, to: 625000, rate: 5, label: "5% (first-time buyer)" },
        { from: 625000, to: 925000, rate: 10, label: "10% (upper band)" },
        { from: 925000, to: 1500000, rate: 12, label: "12% (high value portion)" },
        { from: 1500000, to: null, rate: 12, label: "12% (over Â£1.5m)" },
      ]
    : [
        { from: 0, to: 250000, rate: 0, label: "0% (up to Â£250k)" },
        { from: 250000, to: 925000, rate: 5, label: "5% (from Â£250k to Â£925k)" },
        { from: 925000, to: 1500000, rate: 10, label: "10% (from Â£925k to Â£1.5m)" },
        { from: 1500000, to: null, rate: 12, label: "12% (over Â£1.5m)" },
      ];
...
export default function StampDutyCalculator() {
  ...
  const result = useMemo<StampDutyBreakdown | null>(() => {
    ...
    return calculateSDLT_ENG_WALES_LIKE({ value: propertyValue, firstTimeBuyer: isFirstTimeBuyer });
  ...
  const heroImageSrc = "/images/green-shade-brush-paint-paper-background-removebg-preview.png";
            ...
            <div className="grid lg:grid-cols-6 gap-10 items-start">
              ...
              <div className="lg:col-span-3">
                ...
                >
                  ...
                  <div>
                    <label className="text-sm font-medium text-foreground mb-2 block">Nation</label>
                    <select
                      value={nation}
                      onChange={(e) => setNation(e.target.value as HomeNation)}
                    ...
                    >
                      <option value="england_northern_ireland">England & Northern Ireland</option>
                  ...
                  <div className="bg-accent/10 border border-accent/20 rounded-2xl p-5">
                    ...
                    <p className="text-xs text-muted-foreground mt-2 leading-relaxed">
                      Estimates only. Future API can apply any additional rules (e.g. additional properties).
                    </p>
                  </div>
              ...
              <div className="lg:col-span-3 space-y-6">
                <div className="bg-gradient-to-br from-primary to-phoenix-orange-dark text-primary-foreground rounded-3xl p-8">
                  <p className="text-primary-foreground/70 text-sm mb-1">Estimated Total</p>
                  <p className="text-4xl font-bold">
                    {nation === "england_northern_ireland" && result ? formatCurrency(result.total) : "â"}
                  </p>
                  <p className="text-sm text-primary-foreground/80 mt-3 max-w-xl">
                    This preview is based on simplified bands for England & Northern Ireland.
                  </p>
                </div>

                <div className="bg-card border border-border rounded-3xl p-7">
                  <div className="flex items-start gap-3">
                    <BadgeCheck className="text-accent shrink-0 mt-1" size={20} />
                    <div>
                      <h3 className="text-xl font-bold text-foreground">Breakdown</h3>
                      <p className="text-muted-foreground mt-1 text-sm">
                        API-ready structure: `bands[]` with `label`, `rate`, and `amount`.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-6">
                    {nation !== "england_northern_ireland" ? (
                      <div className="rounded-2xl border border-border bg-background p-5 flex items-start gap-3">
                        <HelpCircle className="text-accent shrink-0 mt-1" size={18} />
                        <div>
                          <p className="font-semibold text-foreground">Wales coming soon</p>
                          <p className="text-sm text-muted-foreground mt-1 leading-relaxed">
                            Wales uses Land Transaction Tax rules. Plug in your API response for a full breakdown.
                          </p>
                        </div>
                      </div>
                    ) : result ? (
                            ...
                            <div className="flex items-start justify-between gap-4">
                              <div>
                                ...
                                <p className="text-xs text-muted-foreground mt-1">
                                  ...
                                  {band.to === null ? "Above" : formatCurrency(band.to)}
                                </p>
                              </div>
                              <div className="text-right">
                                <p className="text-xs text-muted-foreground">Rate</p>
                                <p className="text-2xl font-bold text-accent">{band.rate}%</p>
                              </div>
                            </div>
                            <div className="mt-4 flex items-center justify-between">
                              <span className="text-sm text-muted-foreground">Band Amount</span>
                ...
                <div className="rounded-3xl border border-accent/20 bg-accent/5 p-7 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1 leading-relaxed">
                      For accurate SDLT/LTT calculations, connect your official data and rules API.
                      This UI is designed so you can swap `result` with your API response easily.
                    </p>
                  </div>
                </div>
...
function ChevronRightIcon() {
  return <span className="inline-flex items-center justify-center">â</span>;
}
