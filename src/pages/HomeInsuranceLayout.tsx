...
interface HomeInsuranceLayoutProps {
  service: ServiceItem;
}

const HomeInsuranceLayout = ({ service }: HomeInsuranceLayoutProps) => {
  return (
...
function HeroSection({ service }: { service: ServiceItem }) {
          ...
          >
            <div className="bg-card/80 backdrop-blur-sm rounded-2xl p-8 border border-border shadow-xl">
              ...
              <div className="space-y-3">
                {["Fire & Storm Damage", "Theft Protection", "Accidental Damage", "Flood Cover"].map(
                  (item) => (
...
function InfoCardsSection({ service }: { service: ServiceItem }) {
  const { ref, visible } = useScrollReveal();
...
function FeaturesSection({ service }: { service: ServiceItem }) {
  const { ref, visible } = useScrollReveal();
    ...
    <section ref={ref} className="py-16 md:py-24 bg-phoenix-green-light">
      <div className="container max-w-6xl mx-auto px-4">
        ...
        >
          ...
          <h2 className="font-script text-3xl md:text-4xl text-foreground">What's Included</h2>
        </div>
...
function ConsiderationsSection({ service }: { service: ServiceItem }) {
  const { ref, visible } = useScrollReveal();
    ...
    <section ref={ref} className="py-16 md:py-24 bg-background">
      <div className="container max-w-6xl mx-auto px-4">
        ...
        >
          ...
          <h2 className="font-script text-3xl md:text-4xl text-foreground">Before You Buy</h2>
        </div>
...
function FAQSection({ service }: { service: ServiceItem }) {
  const { ref, visible } = useScrollReveal();
...
function CTASection({ service }: { service: ServiceItem }) {
  const { ref, visible } = useScrollReveal();
...
export default HomeInsuranceLayout;
